...
| 消息码 | 方向 | 含义 | 负载 |
| --- | --- | --- | --- |
| `0x0001` | 上位机 -> 下位机 | 尾部夹爪跟随武器头移动 | `int16_t` |
| `0x0002` | 上位机 -> 下位机 | 尾部夹爪跟随长杆移动 | `int16_t` |
| `0x0003` | 下位机 -> 上位机 | 改变武器检测状态 | `int16_t`，转发到 `/vision/weapon_cmd_state_2` |
| `0x0004` | 下位机 -> 上位机 | 改变杆子检测状态 | `int16_t`，转发到 `/vision/pole_cmd_state_2` |
